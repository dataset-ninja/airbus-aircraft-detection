...
with open("./APP_DATA/annotations.csv", "r") as file:
    ...
    for row in csvreader:
        ...
        if image_id in annotations:
            annotations[image_id].append((class_name, geometry))
        else:
            annotations[image_id] = [(class_name, geometry)]
    ...
    for image_id, class_geometries in annotations.items():
        with open(f"{anns_folder}/{get_file_name(image_id)}.txt", "w") as txtfile:
            for class_name, geometry in class_geometries:
                txtfile.write(f"{class_name}|{geometry}\n")
